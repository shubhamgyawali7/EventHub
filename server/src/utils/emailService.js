...
const transporter = nodemailer.createTransport({
  ...
  // Adding these helps prevent connection hanging in cloud environments
...
export const sendVerificationEmail = async (userEmail, clubName) => {
  ...
  console.log(`📧 Attempting to notify: ${userEmail} regarding ${clubName}`);
  ...
  try {
    ...
    console.log("✅ Verification notification sent. Message ID:", info.messageId);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error("❌ Email failed to send:", error.message);
    // Log the full error in development to catch things like "Invalid Login"
